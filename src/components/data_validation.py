        ...
        try:
            ...
            for col in all_columns:
                if col not in schema_columns:
                    ...
                    with open(
                        self.data_validation_config.data_validation_status_filepath, "w"
                    ) as file:
            ...
            logging.info(
                f"validate columns method interrupted due to {CustomException(e,sys)}"
            )
            raise CustomException(e, sys)

    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            validation_status = self.validate_columns(
                data_filepath=self.data_ingestion_artifact.raw_data_path
            )
            ...
            logging.info(
                f"Data validation inititation interrupted due to {CustomException(e,sys)}"
            )
            raise CustomException(e, sys)
