...
class DataTransformation:
    ...
    def get_data_transformation_object(self)-> object:
        try:
            ...
            logging.info(f"Data transformation object creation interrupted due to {CustomException(e,sys)}")
            raise CustomException(e,sys)
        
    def initiate_data_transformation(self)->DataTransformationArtifact:
        try:
            logging.info(f" reading dataframe ")
            train_dataframe = pd.read_csv(self.data_validation_artifact.validated_train_filepath)
            test_dataframe = pd.read_csv(self.data_validation_artifact.validated_test_filepath)
            
            preprocessor = self.get_data_transformation_object()
            target_column = list(read_yaml_file(SCHEMA_FILE_PATH)['TARGET_COLUMN'].keys())[0]
            ...
            logging.info(f"input feature train dataframe and target feature dataframe created successfully")
            ...
            logging.info(f"input feature test dataframe and target feature dataframe created successfully")
            
            logging.info(f"fitting preprocessor object with input train feature dataframe to create tranformer object")
            ...
            save_object(file_path=self.data_transformation_config.tranformation_object_filepath,obj=preprocessor_object)
            logging.info(f"preprocessor object got saved successfully .")
            
            save_numpy_array(file_path=self.data_transformation_config.transformed_train_filepath,array=train_arr)
            save_numpy_array(file_path=self.data_transformation_config.transformed_test_filepath,array=test_arr)
            
            logging.info(f"Transformed train array and test array got saved successfully")
