...
class DataIngestion:
    ...
    def download_dataset(self,download_url):
        try:
            ...
            if not os.path.exists(self.data_ingestion_config.downloaded_data_filepath):
                request.urlretrieve(url=download_url,filename=self.data_ingestion_config.downloaded_data_filepath)
            logging.info(f"dataset got downloaded and saved into path : {self.data_ingestion_config.downloaded_data_filepath}")  
            return self.data_ingestion_config.downloaded_data_filepath
        except Exception as e:
            logging.info(f" downloading dataset interrupted due to {CustomException(e,sys)}")
    ...
    def extract_dataset(self,downloaded_data_path):
        try:
            ...
            unzip_dir=os.path.dirname(self.data_ingestion_config.extracted_data_filepath)
            ...
            logging.info(f" extracting dataset interrupted due to {CustomException(e,sys)}")
            raise CustomException(e,sys)
        
    def split_extracted_data(self,extracted_data_path):
        try:
            logging.info("spliting extracted dataset into train data and test data set.")
            ...
            os.makedirs(os.path.dirname(self.data_ingestion_config.train_dataset_filepath),exist_ok=True)
            os.makedirs(os.path.dirname(self.data_ingestion_config.test_dataset_filepath),exist_ok=True)
